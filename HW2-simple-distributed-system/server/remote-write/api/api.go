...
func (h *Handler) InitRoutes() {
	...
	// primary
	h.engine.POST("/primary", h.postPrimaryHandler)
	h.engine.PUT("/primary/:id", h.putPrimaryHandler)
	h.engine.PATCH("/primary/:id", h.patchPrimaryHandler)
	h.engine.DELETE("/primary/:id", h.deletePrimaryHandler)
...
func (h *Handler) Run(wg *sync.WaitGroup) error {
	...
	log.Println("Starting remote-write REST API Server at ", addr)
	go func() {
		defer wg.Done()
...
func (h *Handler) Run(wg *sync.WaitGroup) {
	...
	go func() {
		defer wg.Done()
		log.Println("Starting remote-write REST API Server at ", h.server.Addr)
